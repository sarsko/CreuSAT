...
struct Clause<'a>(Vec<&'static Lit>);
pub struct Formula<'a> { clauses: Vec<Clause<'a>>, num_vars: usize, unassigned: Vec<(&'a Lit, &'a Lit)> }
...
fn copy_clause_without<'a>(f: &'a Formula, clause: &'a Clause, literal: &'a Lit) -> Clause<'a> {
...
fn unit_propagate<'a>(f: &'a Formula, literal: &'a Lit) -> Vec<Clause<'a>> {
...
fn next_literal<'a> (f: &'a mut Formula) -> (&'a Lit, &'a Lit) {
    return *f.unassigned.index(0);
}

#[requires((@(f.clauses)).len() < 10000)] // just to ensure boundedness
#[requires(formula_invariant(*f))]
#[ensures(formula_invariant(^f))]
fn inner(f: &'static mut Formula) -> bool {
    if contains_empty(f) {
        return false;
    }
    if consistent(f) {
    ...
    let (pos_lit, neg_lit) = next_literal(f);
    let mut f1 : &'static Formula = &f.clone();
    let mut f2 : &'static Formula = &f.clone();
    return true;
...
#[requires(formula_invariant(*f))]
#[ensures(formula_invariant(^f))]
pub fn solver(f: &'static mut Formula) -> bool {
    if f.num_vars == 0 {
        return true;
    }
    inner(f)
