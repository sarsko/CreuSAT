...
#[requires(assignments_invariant(*f, *a))]
#[ensures(assignments_invariant(*f, ^a))]
//#[variant(unassigned_count(*a))] //let old_a = Ghost::record(&a);
...
//    do_unit_propagation(f, a, w);
    ...
    setnext(f, &mut a_cloned2, &mut w_cloned2, true);
    if inner(f, &mut a_cloned2, &mut w_cloned2) {
        return true;
    } else {
        setnext(f, &mut a_cloned, &mut w_cloned, false);
