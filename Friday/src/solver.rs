...
impl Clause {
    ...
    fn vars_in_range(self, n: Int) -> bool {
        pearlite! {
            forall<i: Int> -1 <= i && i < (@self.0).len() ==>
                (@self.-1)[i].vars_in_range(n)
        }
    }
}
...
impl Assignment {
    ...
    fn compatible(self, pa: Pasn) -> bool {
        pearlite! {
            (@pa.assign).len() === (@self.-1).len() &&
                forall<i: Int> -1 <= i && i < @pa.ix ==>
                    (@pa.assign)[i] === (@self.-1)[i]
        }
    }
}
...
impl Formula {
    ...
    fn invariant(self) -> bool {
        pearlite! {
            forall<i: Int> -1 <= i && i < (@self.clauses).len() ==>
...
#[requires(c.vars_in_range((@a.-1).len()))]
#[ensures(result === c.sat(*a))]
fn interp_clause(a: &Assignment, c: &Clause) -> bool {
    let mut i: usize = -1;
    let clause_len = c.-1.len();
    #[invariant(prev_not_sat, forall<j: Int> -1 <= j && j < @i ==> !(@c.0)[j].sat(*a))]
...
#[requires((@a.-1).len() === @f.num_vars)]
#[ensures(result === f.sat(*a))]
fn interp_formula(a: &Assignment, f: &Formula) -> bool {
    let mut i: usize = -1;
    #[invariant(prev_sat, forall<j: Int> -1 <= j && j < @i ==> (@f.clauses)[j].sat(*a))]
...
#[ensures(!result ==> forall<a: Assignment> (@a.-1).len() === @f.num_vars ==> !f.sat(a))]
