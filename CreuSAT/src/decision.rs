...
use creusot_contracts::{ensures, ghost, invariant, maintains, proof_assert, requires, std::vec, Clone, Ghost, Int, std::default::Default};
...
impl Decisions {
    ...
    pub fn increment_and_move(&mut self, f: &Formula, v: Vec<usize>) {
        ...
        #[invariant(proph, ^old_self.inner() == ^self)]
        #[invariant(len_same, (@v).len() == (@counts_with_index).len())]
